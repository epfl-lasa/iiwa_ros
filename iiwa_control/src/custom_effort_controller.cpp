...
#define add_result(name)                                    \
  {                                                         \
    ...
    for (unsigned int i = 0; i < controllers_.size(); i++) {
      ...
      if (t & robot_controllers::IOType::Force)
        add_result(force_);
      if (t & robot_controllers::IOType::Torque)
        add_result(torque_);
    }
#undef add_result
...
bool CustomEffortController::init(hardware_interface::EffortJointInterface* hw, ros::NodeHandle& n) {
  ...
  if (operation_space_ == "task") {
    ...
    std::string robot_name_ = ns.substr(0, 1) == "/" ? ns.substr(1, ns.size() - 1) : "/" + ns;
    ROS_INFO("CustomEffortController ROBTO NAMTE %s", robot_name_.c_str());
    n.param<std::string>("params/end_effector", end_effector, robot_name_ + "_link_ee");
  ...
  if (symbols_structure.getType() == XmlRpc::XmlRpcValue::TypeStruct) {
    for (XmlRpc::XmlRpcValue::iterator i = symbols_structure.begin(); i != symbols_structure.end(); ++i) {
      ...
      for (size_t k = 0; k < sub.size(); k++) {
        ...
        if (is_sum)
          static_cast<WeightedSumController*>(controllers[name].get())->AddController(std::move(controllers[sub[k]]));
        else
  ...
  if (ctrl_size == 1) {
    ...
    for (unsigned int i = 0; i < ctrl_size; i++) {
      static_cast<WeightedSumController*>(controller_.get())->AddController(std::move(controllers[ctrl_names[i]]));
...
void CustomEffortController::update(const ros::Time& time, const ros::Duration& period) {
  ...
  if (operation_space_ == "task") {
    ...
    if (null_space_control_) {
      Eigen::VectorXd null_space_signal = null_space_Kp_ * (null_space_joint_config_ - curr_robot_state.position_)
  ...
  } else { // regular controller, directly control joints' effort
...
bool CustomEffortController::updateWeight(iiwa_tools::UpdateWeight::Request& request,
  ...
  if (weightedSumController) {
    ...
    if (it != ctrl_names_.end()) {
      ...
      ROS_WARN_STREAM("[CustomEffortController]: Could not find controller with name " << request.controller_name.data
                                                                                       << ".");
  } else
    ROS_WARN(
