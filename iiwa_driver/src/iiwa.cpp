...
Iiwa::Iiwa(ros::NodeHandle& nh) { init(nh); }
...
Iiwa::~Iiwa() {
  ...
  if (_fri_message_data) delete _fri_message_data;
}
...
void Iiwa::init(ros::NodeHandle& nh) {
  ...
  // add namespace
  if (_publish_commanding_status) { _commanding_status_pub.init(_nh, _ns + "/commanding_status", 100); }
  ...
  if (_init_fri()) _initialized = true;
  else
    _initialized = false;
}
...
void Iiwa::_init() {
  ...
  _kuka_position_command.resize(_num_joints);
  _kuka_effort_command.resize(_num_joints);
  ...
  while (urdf_string.empty()) {
    ROS_INFO_ONCE_NAMED("Iiwa",
                        "Iiwa is waiting for model"
  ...
  if (urdf_model_ptr == nullptr)
    ROS_WARN_STREAM_NAMED("Iiwa",
                          "Could not read URDF from '" << _robot_description
  ...
  for (size_t i = 0; i < _num_joints; ++i) {
    ...
    hardware_interface::JointStateHandle joint_state_handle(_joint_names[i],
                                                            &_joint_position[i],
                                                            &_joint_velocity[i],
                                                            &_joint_effort[i]);
    ...
    if (has_soft_limits) {
      joint_limits_interface::PositionJointSoftLimitsHandle joint_limits_handle(joint_position_handle,
                                                                                limits,
                                                                                soft_limits);
    ...
    if (has_soft_limits) {
      joint_limits_interface::VelocityJointSoftLimitsHandle joint_limits_handle(joint_velocity_handle,
                                                                                limits,
                                                                                soft_limits);
...
bool Iiwa::_read(const ros::Duration& ctrl_duration) {
  ...
  for (size_t i = 0; i < _num_joints; i++) {
    ...
    _joint_velocity[i] =
        filters::exponentialSmoothing((_joint_position[i] - _joint_position_prev[i]) / ctrl_duration.toSec(),
                                      _joint_velocity[i],
                                      0.2);
...
void Iiwa::_write(const ros::Duration& ctrl_duration) {
...

  {
    ...
    std::lock_guard<std::mutex> lock_guard(_mutCommand);
    if (_robot_state.getClientCommandMode() == kuka::fri::TORQUE) {
      ...
      // compensation.
      for (size_t i = 0; i < _num_joints; i++) {
        _kuka_position_command.at(i) = _joint_position.at(i) + 0.1 * std::sin(ros::Time::now().toSec());
      }
...
void Iiwa::_disconnect_fri() {
  if (_fri_connection.isOpen()) _fri_connection.close();
}
...
bool Iiwa::_read_fri(kuka::fri::ESessionState& current_state) {
  ...
  if (!_fri_message_data->decoder.decode(_fri_message_data->receiveBuffer, _message_size)) { return false; }
...
bool Iiwa::_write_fri() {
  ...
  if (_fri_message_data->lastSendCounter >= _fri_message_data->monitoringMsg.connectionInfo.receiveMultiplier) {
    ...
    if (!_fri_message_data->encoder.encode(_fri_message_data->sendBuffer, _message_size)) { return false; }
...
void Iiwa::_loopUpdateControl() {
  ...
  while (!_threadTerminate && ros::ok()) {
    // Control rate
    rate.sleep();
    //elapsed_time = rate.cycleTime();
    timeNow = ros::Time::now();
    elapsed_time = timeNow - timePrev;
    timePrev = timeNow;
    ...
    if (_firstRead) {
      ...
      // Update command for asynchronous write
      {
        std::lock_guard<std::mutex> lock_guard(_mutCommand);
        _kuka_position_command = _joint_position_command;
        _kuka_effort_command = _joint_effort_command;
      }
    }
  }
}
