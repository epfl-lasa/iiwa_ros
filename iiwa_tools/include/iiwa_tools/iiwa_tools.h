...
class IiwaTools
{
 public:
    ...
    void init_rbdyn(const std::string &urdf_string,
    ...
    Eigen::VectorXd perform_ik(const EefState &ee_state,
                               const RobotState &seed_state = RobotState());
    Eigen::MatrixXd jacobian(const RobotState &robot_state);
    Eigen::MatrixXd jacobian_deriv(const RobotState &robot_state);
    std::pair<Eigen::MatrixXd, Eigen::MatrixXd> jacobians(
        const RobotState &robot_state);
    Eigen::VectorXd gravity(const std::vector<double> &gravity,
                            const RobotState &robot_state);
 ...
 protected:
    ...
    void _update_urdf_state(mc_rbdyn_urdf::URDFParserResult &rbdyn_urdf,
                            const RobotState &robot_state);
