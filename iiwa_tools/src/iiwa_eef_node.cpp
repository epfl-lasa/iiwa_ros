...
public:
  ...
  bool init(ros::NodeHandle& nh) {
    ...
    std::string robot_name_ = ns.substr(0, 1) == "/" ? ns.substr(1, ns.size() - 2) : "/" + ns;
    nh.param<std::string>("params/end_effector", end_effector, robot_name_ + "_link_ee");
    ...
    _subJointState = nh.subscribe<sensor_msgs::JointState>(ns + "/joint_states",
                                                           QUEUE_SIZE,
                                                           boost::bind(&EefPublisher::updateJointState, this, _1),
                                                           ros::VoidPtr(),
                                                           ros::TransportHints().reliable().tcpNoDelay());
    _subAdditionalOutputs =
        nh.subscribe<iiwa_driver::AdditionalOutputs>(ns + "/additional_outputs",
                                                     QUEUE_SIZE,
    ...
    // Init publishers
    _pubEefState.init(nh, ns + "/eef_state", QUEUE_SIZE);
  ...
  void updateJointState(const sensor_msgs::JointStateConstPtr& msgJointState) {
    ...
    Eigen::MatrixXd jacTPInv(NB_DIM, position.size());
    ...
    Eigen::Vector6d eefWrench = jacTPInv * torque;
    Eigen::Vector6d eefExtWrench;
    ...
    if (_pubEefState.trylock()) {
      ...
      _pubEefState.msg_.angle_self = msgJointState->position[DOF_ANGLE_SELF];
