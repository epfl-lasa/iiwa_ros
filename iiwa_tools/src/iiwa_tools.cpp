...
iiwa_tools::EefState IiwaTools::perform_fk(const RobotState &robot_state)
{
    ...
    for (size_t i = 0; i < _rbd_indices.size(); i++)
    {
        ...
        q_low(i) =
            rbdyn_urdf.limits.lower[rbdyn_urdf.mb.joint(index).name()][0];
        q_high(i) =
            rbdyn_urdf.limits.upper[rbdyn_urdf.mb.joint(index).name()][0];
    }
    ...
    for (size_t i = 0; i < _rbd_indices.size(); i++)
    {
        ...
        jt = wrap_angle(jt);
        // enforce limits
        if (jt < q_low(i)) jt = q_low(i);
        if (jt > q_high(i)) jt = q_high(i);
                                      ...
                                      const RobotState &seed_state)
{
    ...
    Eigen::VectorXd damping =
        Eigen::VectorXd::Ones(_rbd_indices.size()).array() * damp;
    ...
    tf.block(0, 0, 3, 3) = ee_state.orientation.normalized().matrix();
    ...
    if (seeds_provided)
    {
        for (size_t i = 0; i < _rbd_indices.size(); i++)
        {
            ...
            // enforce limits
            if (seed < q_low(i)) seed = q_low(i);
            if (seed > q_high(i)) seed = q_high(i);
    ...
    bool valid =
        _ik->inverseKinematics(rbdyn_urdf.mb, rbdyn_urdf.mbc, target_tf);
    if (valid)
    {
        for (size_t i = 0; i < _rbd_indices.size(); i++)
        {
            ...
            // enforce limits
            if (qref(i) < q_low(i)) qref(i) = q_low(i);
            if (qref(i) > q_high(i)) qref(i) = q_high(i);
        }
    ...
    for (iter = 0; iter < max_iterations; iter++)
    {
        ...
        Eigen::Vector3d rotErr =
            sva::rotationError(rbdyn_urdf.mbc.bodyPosW[_ef_index].rotation(),
                               target_tf.rotation());
        Eigen::Vector6d v;
        v << rotErr, target_tf.translation() -
                         rbdyn_urdf.mbc.bodyPosW[_ef_index].translation();
        ...
        memcpy(ik_solver.params.damping,
               damping.data(),
               _rbd_indices.size() * sizeof(double));
        memcpy(ik_solver.params.slack, slack_vec.data(), 6 * sizeof(double));
        memcpy(
            ik_solver.params.qref,
            zero.data(),
            _rbd_indices.size() *
                sizeof(
                    double));  // we set qref to zero so that we minimize the dq
        memcpy(ik_solver.params.qlow,
               qlow.data(),
               _rbd_indices.size() * sizeof(double));
        memcpy(ik_solver.params.qup,
               qhigh.data(),
               _rbd_indices.size() * sizeof(double));
        ...
        if ((q_prev - qref).norm() < 1e-8) break;
    }
...
Eigen::VectorXd IiwaTools::gravity(const std::vector<double> &gravity,
...
std::pair<Eigen::MatrixXd, Eigen::MatrixXd> IiwaTools::jacobians(
    const RobotState &robot_state)
{
    // copy RBDyn for thread-safety
    // TO-DO: Check if it takes time
    ...
    return std::make_pair(jac.jacobian(rbdyn_urdf.mb, rbdyn_urdf.mbc),
                          jac.jacobianDot(rbdyn_urdf.mb, rbdyn_urdf.mbc));
}

void IiwaTools::init_rbdyn(const std::string &urdf_string,
...
void IiwaTools::_update_urdf_state(mc_rbdyn_urdf::URDFParserResult &rbdyn_urdf,
                                   const RobotState &robot_state)
{
    for (size_t i = 0; i < _rbd_indices.size(); i++)
    {
