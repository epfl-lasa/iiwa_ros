...
Eigen::VectorXd IiwaTools::perform_ik(const EefState& ee_state, const RobotState& seed_state) {
  ...
  for (iter = 0; iter < max_iterations; iter++) {
    ...
    // set params
    for (int r = 0; r < 6; r++) {
      for (int c = 0; c < _rbd_indices.size(); c++) { ik_solver.params.J[r + 1][c] = jac_mat(r, c); }
    }
    ...
    memcpy(ik_solver.params.qref,
           zero.data(),
           _rbd_indices.size() * sizeof(double));// we set qref to zero so that we minimize the dq
    ...
    for (size_t j = 0; j < _rbd_indices.size(); j++) { qref(j) += ik_solver.vars.dq[j]; }
...
void IiwaTools::init_rbdyn(const std::string& urdf_string, const std::string& end_effector) {
  ...
  for (size_t i = 0; i < _rbdyn_urdf.mb.nrJoints(); i++) {
    if (_rbdyn_urdf.mb.joint(i).type() != rbd::Joint::Fixed) _rbd_indices.push_back(i);
  }
...
size_t IiwaTools::_rbd_index(const std::string& body_name) const {
  for (size_t i = 0; i < _rbdyn_urdf.mb.nrBodies(); i++) {
    if (_rbdyn_urdf.mb.body(i).name() == body_name) { return i; }
  }
...
void IiwaTools::_update_urdf_state(mc_rbdyn_urdf::URDFParserResult& rbdyn_urdf, const RobotState& robot_state) {
  for (size_t i = 0; i < _rbd_indices.size(); i++) {
    ...
    if (robot_state.position.size() > i) rbdyn_urdf.mbc.q[rbd_index][0] = robot_state.position[i];
    if (robot_state.velocity.size() > i) rbdyn_urdf.mbc.alpha[rbd_index][0] = robot_state.velocity[i];
    if (robot_state.torque.size() > i) rbdyn_urdf.mbc.jointTorque[rbd_index][0] = robot_state.torque[i];
  }
}
