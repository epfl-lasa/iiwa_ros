...
bool IiwaService::perform_fk(iiwa_tools::GetFK::Request& request, iiwa_tools::GetFK::Response& response) {
  ...
  for (size_t point = 0; point < request.joints.layout.dim[0].size; ++point) {
    ...
    for (size_t i = 0; i < _n_joints; i++) robot_state.position[i] = get_multi_array(request.joints, point, i);
...
bool IiwaService::perform_ik(iiwa_tools::GetIK::Request& request, iiwa_tools::GetIK::Response& response) {
  bool seeds_provided =
      request.seed_angles.layout.dim.size() == 2 && (request.seed_angles.layout.dim[0].size == request.poses.size());
  ...
  for (size_t point = 0; point < request.poses.size(); ++point) {
    ...
    ee_state.translation = {request.poses[point].position.x,
                            request.poses[point].position.y,
                            request.poses[point].position.z};
...
bool IiwaService::get_jacobian(iiwa_tools::GetJacobian::Request& request, iiwa_tools::GetJacobian::Response& response) {
  ...
  for (int i = 0; i < jac_mat.rows(); i++) {
    for (int j = 0; j < jac_mat.cols(); j++) { set_multi_array(response.jacobian, i, j, jac_mat(i, j)); }
  }
...
bool IiwaService::get_jacobian_deriv(iiwa_tools::GetJacobian::Request& request,
  ...
  for (int i = 0; i < jac_deriv_mat.rows(); i++) {
    for (int j = 0; j < jac_deriv_mat.cols(); j++) { set_multi_array(response.jacobian, i, j, jac_deriv_mat(i, j)); }
  }
...
bool IiwaService::get_gravity(iiwa_tools::GetGravity::Request& request, iiwa_tools::GetGravity::Response& response) {
  if (request.joint_angles.size() != _n_joints || request.joint_angles.size() != request.joint_velocities.size()
      || request.joint_angles.size() != request.joint_torques.size()) {
    ROS_ERROR_STREAM("The requested joint size is not the same as the robot's size or some field is missing!");
    return false;
  }
  ...
  if (request.gravity.size() != 3) {
    ...
    for (size_t i = 0; i < 3; i++) { gravity[i] = request.gravity[i]; }
  }
  ...
  for (size_t i = 0; i < _n_joints; i++) { response.compensation_torques[i] = C(i); }
...
bool IiwaService::get_mass_matrix(iiwa_tools::GetMassMatrix::Request& request,
  ...
  RobotState robot_state;
  robot_state.position.resize(_n_joints);
  for (size_t i = 0; i < _n_joints; i++) { robot_state.position[i] = request.joint_angles[i]; }
  ...
  for (int i = 0; i < H.rows(); i++) {
    for (int j = 0; j < H.cols(); j++) { set_multi_array(response.mass_matrix, i, j, H(i, j)); }
  }
...
void IiwaService::_load_params() {
  ...
  n_p.param<std::string>("service/jacobian_deriv_service_name",
                         _jacobian_deriv_service_name,
...
void IiwaService::init() {
  ...
  while (urdf_string.empty()) {
    ROS_INFO_ONCE_NAMED("IiwaService",
                        "IiwaService is waiting for model"
